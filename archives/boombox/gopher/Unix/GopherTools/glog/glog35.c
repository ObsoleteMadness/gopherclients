/*** glog35.c -- analysis tool for Unix gopherd logs ***/
/******************** START CONFIGURATION ***************************/
...
/* #define DETAIL_TOTAL*/  /* uncomment if you WANT detail percentages to 
			      be out out the total number of connections  */
/* #define NOSEARCHTXT */  /* uncomment if you DON'T want to output WHAT  */
                           /*    was searched for                         */
/* #define HCASEMATTERS */ /* uncomment if case matters for hostnames     */
...
#define	GLOG_VERSION "Gopher Log Analyzer 3.5"

/* Version 3.5 ?/?/94 awick
 * Added Domain Reports
 * Added -s? sort of reports and histograms, not plots though
 * Changed some shorts to ints
 * Lowers the case of all hostnames now, unless above uncommented
 * Only keep track of reports need, saves memory and time.
 * Increased speed of adding data (special InsertDates now)
 *
...
byte do_D=0,do_H=0, do_W=0, do_T=0, do_I=0, do_O=0;
...
int SortMult = 1; /*Forward Sort, -1 = Reverse Sort */
byte SortHits = 1; /*Sort by Hits = 1, Names = 0 */
...
#define EXITERROR(str) {fprintf(stderr, (str)); fflush(stderr); exit(1);}
...
/*******************************/
/* Insert dates into tree, if that element is already in the tree
 * then increment the number of hits.  cmp is used to find the location
 * in the tree.
 */

NODE_PTR InsertDates(NODE_PTR tree, GOPHER_PTR data, int cmp(GOPHER_PTR a, GOPHER_PTR b))
{
int i;
...
NODE_PTR temp;
   ...
   i=cmp(data, last->data);

   if (i == 0)
   {
      last->hits++;
#ifndef NODETAIL
      last->llist = InsertDetail(last->llist, data);
#endif
   }
   else
   {
      temp = last; /* because last is going to be reset */
      temp->right = InsertDates(last->right, data, cmp);
   }
...
/*******************************/
int DomainCmp(GOPHER_PTR a, GOPHER_PTR b)
{ 
...
void PlotDay(FILE *rfp, long int num, GOPHER_PTR data)
{
   ...
   {
      fprintf(rfp,"\"%s\" %ld,",Days[data->day-1], num);
   }
}
/*******************************/
void PlotTime(FILE *rfp, long int num, GOPHER_PTR data)
{
   if (data == STARTINFO)
   {
      fprintf(rfp,"set xtics (");
   ...
   {
      fprintf(rfp,"\"%2d\" %ld,",data->hour, num);
   }
}
/*******************************/
void PlotDate(FILE *rfp, long int num, GOPHER_PTR data)
{
   if (data == STARTINFO)
   {
      fprintf(rfp,"set xtics (");
   ...
   else
   {
      if ((data->date == 1) || (data->date == 15) || (num == 1))
         fprintf(rfp,"\"%s/%d\" %ld,",Months[data->month-1], data->date, num);
   }
}
#ifndef NODETAIL
/*******************************/
