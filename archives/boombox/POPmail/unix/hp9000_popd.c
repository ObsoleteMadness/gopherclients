/*
  ...
  "as is" without express or implied warranty.
        ...
        Version 1.5c   Dec 03, 1990, Robert Alberti
...
Changes since the 1.0 release:
        ...
        Just one log file.
12/03/90 RMA
        Changed default log file to none (/dev/null).
        Changed default statistic mailing to 'off'.
...
Notes:
        ...
        *** The password lookup code may not work if you are using
        ...
        strange to C purists. (12/03/90 RMA:  No kidding!  Sheesh! :-> )

Begin:  Modifications to code (12/12/91)--> David Stevenson, Naval Research Lab

        I had to put in some of my own defines to get the program to compile
        properly with the HP-UX (HP 9000/380, HP-UX 7.05) compiler. I also
        had to make a few other changes. All changes are activated by using
        the -DHPUX switch in the compile line. The existing program integrity
        has been preserved (PASCAL ?!).

        Also below, note the compile (build) line to use in compiling this
        program for Hewlett Packard HP-UX Unix systems.

        Bug Fix: NOFLOCK define is used as NOLOCKF (misspelled) - corrected

End:    Modifications to original by --> David Stevenson, Naval Research Lab

To build:

        cc -o popd { options } popd.c

    cc -O -s -o popd -DHPUX -DNOFLOCK popd.c -lBSD   <<-- use for HP-UX ...DPS.

Compile options:
        -DAUX           ( for A/UX or maybe system V )
        -DHPUX          ( for HP-UX 7.05 (and later(?)))
...
This changes its behavior so it writes debug information to the terminal.
...
If you want to keep a log file, you will wish to use

        % su
        Password:
        # ./popd -l<full pathname to file>

Note that the -l parameter is ignored when -d is invoked, and the log is
...
Options:
        ...
        -l xxx  Write log to file xxx.  Default is no log (/dev/null).
                This option is ignored when -d is selected, the log
        ...
        -s      Turn on mailing statistics to "POPADMIN". Normally does not
                mail out daily statistics.
...
Problems?:
        You can instruct the daemon to write to a log file with the -l option.

        popd -l/usr/adm/popd.log

        The logfile gets informational messages such as who read their mail
        When -d is selected the -l option is ignored and the log goes
...
Known problems:
        ...
        (2) I'm not a wizard on security problems caused by programs
                ...
                (b) There's no limit to the number of commands sent to
                    ...
                    time.
                (c) The encrypted password can be decrypted.  Additionally,
                    the decrypted password is displayed during debug mode,
                    which is why debug mode output cannot be saved to a file.
                    
           I'd be glad to receive any hints or improvements.

        (3) The log file can grow without bound.  If you choose to have
                a log file, you must trim it back periodically.
...
#define SIGXCPU        99               /* SIGXCPU and SIGXFSZ signals are */
#define SIGXFSZ        99               /* not defined (HP-UX), so assign */
#endif                                  /* harmless values to them */
...
#define LOGFILEDIR      "/dev"
#define LOGFILENAME     "null"
...
code
    ...
    IF (not debug) LOG "Logging to file %s.\n", ofile ); FI
    ...
    ELSE
     ...
     IF( LockAcquire( lockFile ) is Okay )
        IF( FLockAcquire( inmailFile ) is Okay )
           ...
           holdf = open ( tempmailFile, O_RDONLY );
...
code
   call sprintf( mbox, "%s/%s", spfile, PopArg[1] );
